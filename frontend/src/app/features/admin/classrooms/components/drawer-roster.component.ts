  ...
  template: `
    <bf-drawer
      ...
      <div class="px-3 py-4 space-y-4">
        ...
        <div
          *ngIf="isDropped"
          class="rounded bg-amber-50 border border-amber-200 px-3 py-2 text-amber-900 text-sm">
          Classroom is dropped. You can view the roster history but cannot enroll until it’s reactivated.
        </div>

        <!-- Restore banner (ACTIVE only) -->
        <div
          *ngIf="!isDropped && droppedCount > 0"
          class="rounded border border-sky-200 bg-sky-50 text-sky-900 px-3 py-2 text-sm">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <span>{{ droppedCount }} previously dropped enrollment(s) found.</span>
            <div class="flex flex-wrap gap-2">
              <button
                type="button"
                class="px-2 py-1 rounded bg-sky-600 text-white hover:bg-sky-700 text-xs"
                (click)="restore.emit()">
                Restore
              </button>
              <button
                type="button"
                class="px-2 py-1 rounded border hover:bg-sky-200 text-xs"
                (click)="showDroppedList = !showDroppedList">
                View
              </button>
              <button
                type="button"
          ...
          <div
            *ngIf="showDroppedList && (droppedItems?.length || 0) > 0"
            class="mt-3 space-y-2">
            <div
              *ngFor="let d of droppedItems"
              class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border rounded p-2">
              <div class="text-sm">
                {{ d.student?.name }}
                <span class="text-xs text-slate-600">— {{ d.student?.email }}</span>
                <div class="text-xs text-slate-500" *ngIf="d.droppedAt">
                  Dropped: {{ d.droppedAt | date:'short' }}
                </div>
              </div>
              <button
                type="button"
                class="btn btn-danger btn-sm w-full sm:w-auto"
                (click)="discard.emit(d.id)">
                Discard
              </button>
            </div>
          </div>
        ...
        <div *ngIf="!isDropped" class="space-y-2">
          <h3 class="text-sm font-medium mt-1">Add a student</h3>
          ...
          <form
            ...
            <div>
              ...
              <div *ngIf="studentsLoading" class="text-xs text-slate-500 mt-1">
                Loading students…
              </div>
            </div>
        ...
        <div class="space-y-2">
          <div
            *ngFor="let e of roster"
            class="border rounded p-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <div class="text-xs mt-1">
                ...
                <span *ngIf="e.enrolledAt" class="ml-2">
                  Enrolled: {{ e.enrolledAt | date:'short' }}
                </span>
              </div>
            </div>
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              <button
                ...
                (click)="openGradesFor(e)">
                Grades
              </button>

              <button
                *ngIf="!isDropped && e.status==='ACTIVE'"
                type="button"
                class="btn btn-danger btn-sm w-full sm:w-auto"
...
export class DrawerRosterComponent {
  ...
  @Input() form!: FormGroup; // { onlyFree: boolean; studentId: number|null }
