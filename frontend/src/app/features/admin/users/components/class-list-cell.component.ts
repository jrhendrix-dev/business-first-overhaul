// src/app/features/admin/users/components/class-list-cell.component.ts (path as you have it)
    ...
    <div class="relative flex flex-col gap-2 sm:flex-row sm:items-center">
      ...
      <ng-template #hasData>
        <div class="flex items-center gap-2">
          <!-- first chip -->
          <span
            class="inline-block px-2 py-0.5 rounded-full border text-xs max-w-[9rem] sm:max-w-[12rem] truncate"
                ...
                <div class="max-h-56 overflow-auto">
                  <div
                    *ngFor="let c of classes"
                    class="px-2 py-1 text-sm hover:bg-slate-50 rounded">
                    {{ c.name }}
                  </div>
                </div>
              </div>
      ...
      <a
        ...
        class="mt-1 sm:mt-0 sm:ml-auto inline-flex items-center justify-center rounded-md border px-2 py-1 text-xs
               text-slate-600 hover:bg-slate-50 w-full sm:w-auto text-center"
